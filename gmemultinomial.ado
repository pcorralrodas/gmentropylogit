...
program define gmemultinomial, eclass
	...
	version 11.2
	syntax varlist(min=2 numeric fv) [if] [in] [,GENerate(string) NOCons BASEoutcome(numlist) Priors(varlist numeric min=1)]
	...
	if ("`priors'"!=""){
		...
		local nprior: list sizeof priors
		assert (`numdep'-1)==`nprior'
		
		foreach x of local priors{
			replace `touse' = 0 if missing(`x')
		}
		tempvar check
		egen `check' = rsum(`priors')
		assert `check'<=1
	}
	else{
		local _nn = `numdep'-1
