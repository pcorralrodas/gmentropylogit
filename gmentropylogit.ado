...
program define gmentropylogit, eclass
	version 11.2
	syntax varlist(min=2 numeric fv) [if] [in] [,Mfx GENerate(string)]
				  ...
				  
{
	...
	newvar[.,1]=exp(X*beta'):/(J(rows(X),1,1)+exp(X*beta'))
...
function MEdiscrete(todo, R, Y, X, v, L, g, H)
{
					 ...
					 
{
	...
	real matrix  Y, X, vcov, dfdz, dfdB, G, dummy, x1, x0, vcov2
...
// MFX: Generate Probabilities
	...
	p = exp(X*beta'):/(exp(X*beta')+J(rows(X),1,1))
	...
	for(i=1; i<=cols(dummy); i++){
		...
		if (dummy[,i]==1) {
			...
			p1 = exp(x1*beta'):/(exp(x1*beta')+J(N,1,1))
			
			p0 = exp(x0*beta'):/(exp(x0*beta')+J(N,1,1))
...
S = -(sum(p:*log(p))+sum((J(rows(p),1,1)-p):*log(J(rows(p),1,1)-p)))
...
// Normalized entropy
P=exp(-X*beta'):/(J(rows(Y),1,1)+exp(-X*beta'))
S=-(sum(P:*log(P))+sum((J(rows(P),1,1)-P):*log(J(rows(P),1,1)-P)))
